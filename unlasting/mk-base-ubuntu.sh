...
if [ "$TARGET" == "gnome" ]; then
    ...
    DEBIAN_FRONTEND=noninteractive apt install -y rsyslog sudo dialog apt-utils ntp evtest acpid udev
...
# Create User
useradd -G sudo -m -s /bin/bash sora
passwd sora <<IEOF
sora
sora
IEOF
gpasswd -a sora video
gpasswd -a sora audio
passwd root <<IEOF
sora
sora
IEOF
